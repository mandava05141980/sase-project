...
file_path = '/Lakshmi/Autodesk/KT/Python/Autodesk-Projects/sase-project/AWS-Corp-VPN/corp-prod/aws_accounts1.xlsx'

# Read only the 'app' column and other required columns from the Excel file
# Specify the dtype for the 'app' column separately
df = pd.read_excel(file_path, usecols=['app', 'ip', 'serviceid', 'vpc',  'env'], dtype={'app': str})

# Group by 'app' and aggregate 'ip', 'group', 'serviceid' columns as lists
...
def create_security_rule_wrapper(app, vpc, serviceid, env):
    ...
    create_security_rule(app, serviceid, vpc, env)
...
for index, row in grouped.iterrows():
    ...
    create_security_rule_wrapper(app, row['serviceid'], row['vpc'], row['env'])
